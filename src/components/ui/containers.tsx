...
>(({ className, children }, ref) => {
  ...
  return (
    <div
      ref={ref}
      className={cn(
        `max-w-[1680px] border-b border-gray-200 bg-white z-40 px-3 sm:px-4 lg:px-8 flex justify-between items-center mx-auto relative fixed top-[62px] sm:top-[72px] left-0 right-0 h-[60px] sm:h-[76px] ${
...
const DashboardContainerContent = React.forwardRef<
  HTMLDivElement,
  DashboardProps
...
const FormsContainer = React.forwardRef<
  HTMLDivElement,
  React.HTMLAttributes<HTMLDivElement>
>(({ className, children }, ref) => {
  const { state } = useSidebar();
...
const LandingTitle = ({
  ...
  return (
    <div className="flex mx-auto flex-col justify-between items-center gap-2">
      ...
      <Reveal1>
        <h2 className="text-center text-black font-[600] text-[17.5px] md:text-[28px]">
          {title}
        </h2>
      </Reveal1>
