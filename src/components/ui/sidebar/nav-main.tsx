...
export function NavMain({ items, type }: any) {
  ...
  const isNavActive = (pathname: string, itemUrl: string) => {
    ...
    if (itemUrl.startsWith("/dashboard/wallet"))
      return pathname.startsWith("/dashboard/wallet");
    if (itemUrl.startsWith("/dashboard/events"))
      return pathname.startsWith("/dashboard/events");
    if (itemUrl.startsWith("/dashboard/overview"))
      return pathname.startsWith("/dashboard/overview");
    if (itemUrl.startsWith("/dashboard/listing"))
      return pathname.startsWith("/dashboard/listing");
    if (itemUrl.startsWith("/dashboard/service"))
      return pathname.startsWith("/dashboard/listing");
    if (itemUrl.startsWith("/dashboard/check-in"))
      return pathname.startsWith("/dashboard/check-in");
          ...
          {item.title === "Logout" ? (
            <div>
              {state === "collapsed" ? (
                <LogOut
                  ...
                  className="w-6 h-6 pl-1 ml-1 mt-2 cursor-pointer text-red-700"
                />
              ) : (
                <div
                  onClick={() => setOpen(true)}
            ...
            >
              <Link href={item?.url}>
                ...
                {item?.orders > 0 && (
                  <p className="text-green-700 bg-green-100 w-6 text-center ml-4 rounded-md">
                    {item?.orders}
                  </p>
                )}
              </Link>
