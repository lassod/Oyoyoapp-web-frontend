...
import { formOrderTicket } from "@/app/components/schema/Forms";
...
const TicketSummary = ({ ticket, event, guest = false, currency }: any) => {
  ...
  console.log(totalPercentageFee);
  ...
  const submitTicket = (data: any) =>
    mutation.mutate(data, {
      onSuccess: (response: any) => {
        ...
        if (response.data.data.paymentGateway === "PAYSTACK") router.push(response.data.data.authorization_url);
  ...
  return (
    <Dashboard className={`${guest ? "lg:px-16" : "mt-16"}`}>
                    ...
                    <FormField
                      ...
                      render={({ field: formField }) => (
                        ...
                        >
                          ...
                          <input type='hidden' {...form.register(`form_response.${index}.label`)} value={field.label} />
                          <input
                            type='hidden'
      ...
      {isResponse && <TicketSuccessModal setIsResponse={setIsResponse} guest={guest} event={{ ...event, orderId }} />}
      {showPreview && (
        <PreviewTerms
