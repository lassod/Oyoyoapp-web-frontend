...
import { formatDate, handleShare, scrollToTop, shortenText } from "@/lib/auth-helper";
...
const ViewEvent = ({ event, setTicket }: any) => {
  ...
  if (event?.completed)
    ticketsWithPlans = fetchedEvent?.Event_Tickets?.filter(
      (ticket: any) => ticket?.UserId?.toString() === session?.user?.id
    ) // Filter tickets by UserId
      ?.map((ticket: any) => {
        const matchingPlan = eventPlans?.find((plan: any) => plan.id === ticket.EventPlanId);
        return matchingPlan ? { ...ticket, EventPlan: matchingPlan } : null; // Include matching tickets only
      })
      ?.filter((ticket: any) => ticket !== null); // Remove null entries
    ...
    <>
      <div className='relative mx-auto pb-5 mt-[115px] sm:mt-36'>
        ...
        <DashboardContainerContent type={1}>
          <EventSummary event={{ ...event, Event_Plans: eventPlans }} category={category} guest={false} />
                ...
                <div className='flex flex-col gap-[10px] pt-2 pb-6'>
                  ...
                  <div className='flex justify-between mt-3 gap-6'>
                    <div className='flex flex-col gap-3'>
                      <div>
                        <p>Event:</p>
                        <p className='text-black mt-[6px] font-medium'>{shortenText(event?.title, 15)}</p>
                      </div>
                      <div>
                        <p>Reference:</p>
                        <p className='text-black mt-[6px] font-medium'>{ticket?.ref}</p>
                      </div>
                      <div>
                        <p>Date:</p>
                        <p className='text-black mt-[6px] font-medium mb-10'>{formatDate(event?.date)}</p>
                      </div>
                    </div>
                    <div className='flex flex-col gap-3'>
                      <div>
                        <p>Ticket Type:</p>
                        <p className='text-black mt-[6px] font-medium'>{ticket?.Event_Plans?.name} </p>
                      </div>
                      <div>
                        <p>Amount:</p>
                  ...
                  ticketsWithPlans?.map((item: any) => (
                    ...
                    >
                      <div className='flex flex-col'>
                        <div>
                          ...
                          <span className='text-red-700 font-semibold text-lg'>
                            {item.EventPlan.symbol} {item.EventPlan.price.toLocaleString()}
                          </span>
                        </div>
                      ...
                      <div className='flex items-center gap-3 mt-4'>
                        <p onClick={() => setOrderId(item.id)} className='text-green-700 font-[500] cursor-pointer'>
                          View ticket
                        </p>
                      </div>
                    </div>
              ...
              <div className='bg-white p-4 md:p-5 flex flex-col gap-4'>
                    ...
                    <div className='flex items-center gap-3 mt-4'>
                      <button
                        ...
                        disabled={
                          ...
                          event.capacity - Object.values(ticketCounts).reduce((sum, count) => sum + count, 0)
                        }
                      >
                ...
                <Button
                  ...
                  disabled={
                    ...
                    Object.values(ticketCounts).reduce((sum, count) => sum + count, 0) > event.capacity
                  }
                >
                  Proceed
                </Button>
