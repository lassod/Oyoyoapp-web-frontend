...
const View = ({ params }: { params: { orderId: string } }) => {
  ...
  const { data: service, status: serviceStatus } = useGetService(
    data?.orderTypeDetail?.ServiceId
  );
  ...
  useEffect(() => {
    if (transaction && user) {
      setData(transaction);
      if (transaction?.transactionRef?.[0]?.recieverId === user?.id)
        setIsVendor(true);
    ...
    if (data?.transactionRef[0]?.paymentGateway === "PAYSTACK")
      ...
      router.push(
        data?.transactionRef[0]?.authorization_url ||
          data?.transactionRef[0]?.paymentGatewayTransitionInfo
            ?.authorization_url
      );
  ...
  useEffect(() => {
    if (data) {
      ...
      if (data?.totalAmount + data?.transactionRef?.[0]?.transactionFeeTotal)
        total =
          data?.totalAmount + data?.transactionRef?.[0]?.transactionFeeTotal;
  ...
  if (status !== "success" && serviceStatus !== "success")
    ...
    <Dashboard className="relative mx-auto pt-[145px]">
      <DashboardHeader>
          ...
          >
            ...
            data?.transactionRef[0]?.purpose !== "TICKET_PURCHASE" ? (
              ...
              <span className="max-w-[400px] gap-[10px] flex">
                ...
                <Button
                  onClick={handlePrint}
                  className="flex sm:px-5 justify-center items-center gap-[8px]"
                >
                  Print Receipt
              ...
              <Button
                onClick={handlePrint}
                className="flex mr-0 sm:px-5 justify-center items-center gap-[8px]"
              >
                Print Receipt
              </Button>
            )}
        ...
        {data?.orderStatus === "CONFIRMED" && !isVendor ? (
          ...
          <Button
            onClick={handlePrint}
            className="hidden sm:flex mr-0 sm:px-5 justify-center items-center gap-[8px]"
          >
            Print Receipt
            <PrinterIcon className="h-5 w-5" />
          </Button>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-[100px,1fr] sm:flex-row gap-2 items-center">
                <div className="w-full sm:w-[100px] h-[130px] sm:h-[100px] overflow-hidden rounded-lg">
                  <Image
                    ...
                    src={
                      item?.Event_Plans?.Events?.media[0] || service?.media[0]
                    }
                    width={100}
                    height={100}
                    alt="Image"
          ...
          <div className="flex bg-white flex-col gap-4 rounded-md border border-gray-200 px-4 py-3">
            <div className="flex flex-col gap-2">
              ...
              <div className="flex justify-between gap-4">
                ...
                <p className="text-black">{`${data?.symbol} ${
                  data?.transactionRef?.[0]?.transactionFeeTotal?.toLocaleString() ||
                  0
                }`}</p>
              </div>
              <div className="flex justify-between gap-4">
            ...
            <div className="flex justify-between gap-4">
              ...
              <p className="text-red-700 font-medium">{`${data?.symbol} ${
                total.toLocaleString() || 0
              }`}</p>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-4 py-4 border-b border-gray-200">
            ...
            <div className="flex items-center gap-2">
              ...
              <p className="text-black font-medium">
                ...
                {data?.user?.last_name || data?.user?.username}
              </p>
            </div>
          </div>
          <div className="flex flex-col gap-4 py-4 border-b border-gray-200">
