...
const Orders = ({ params }: any) => {
  ...
  const getColumnsForActiveTab = () => {
    return tab === "all-orders"
      ? OrdersCol
      : OrdersCol.filter(({ accessorKey }: any) => accessorKey !== "customer");
  };
      ...
      {tab !== "event-order" && (
        <div className="flex flex-col">
          ...
          <div className="grid gap-4 grid-flow-row-dense sm:grid-cols-2 md:grid-cols-4 border-b border-gray-200 pt-5 pb-8 m-0">
            <CardWallet
              title="Active Orders"
              header={vendorStats?.activeOrders || 0}
            />
            <CardWallet
              title="Active Packages"
              header={vendorStats?.activePackages || 0}
            />
            <CardWallet
              title="Pending Funds"
              header={`${user?.Wallet?.symbol || "₦"} ${
                vendorStats?.pendingFunds?.toLocaleString() || 0
              }`}
            />
            <CardWallet
              title="Avg Rating"
              header={vendorStats?.avgRating || 0}
            />
            <CardWallet
              title="Total Sold"
              header={`${user?.Wallet?.symbol || "₦"} ${
