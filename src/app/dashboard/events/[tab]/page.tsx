...
const Events = () => {
  ...
  const { data: attending, status: attendingStatus } =
    useGetUserAttendingEvents();
  ...
  const renderEventView = () => {
    const views: any = {
      ...
      "/dashboard/events/completed": event && (
        <ViewEvent
          event={{ ...event, completed: true }}
          setTicket={setTicket}
        />
      ),
      "/dashboard/events/aievent-planner": event && (
        <AiEventPlanner event={event} />
      ),
      "/dashboard/events/table-arrangement": event && (
        <TableArrangement event={event} />
      ),
      "/dashboard/events/manage-access": event && (
            ...
            <>
              {tab.showStats && (
                <div className="grid gap-4 grid-flow-row-dense sm:grid-cols-2 md:grid-cols-4 border-b border-gray-200 pt-5 pb-8 m-0">
                  <CardWallet
                    title="Total Tickets"
                    header={eventStats?.totalTickets?.toString() || "--"}
                  />
                  <CardWallet
                    title="Active Events"
                    header={eventStats?.activeEvents?.toString() || "--"}
                  />
                  <CardWallet
                    title="Total Unsold"
                    header={eventStats?.totalUnsold?.toString() || "--"}
                  />
                  <CardWallet
                    title="Total Sold"
                    header={eventStats?.totalSold?.toString() || "--"}
                  />
                </div>
              )}
              {tab.showFilter && (
                <FilterMenuEvent item={tab} setFilters={setFilters} />
              )}
              <AllEvents
                status={tab.status}
