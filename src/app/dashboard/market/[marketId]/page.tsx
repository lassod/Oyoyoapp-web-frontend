...
import { BadgeCheck, Loader2, Package, Star, UserPlus } from "lucide-react";
...
const ViewMarket = ({ params }: any) => {
  ...
  const { data: vendorService, isLoading: vendorServiceLoading } =
    useGetVendorServices(service?.VendorId);
  const { data: rating, isLoading: ratingLoading } = useGetVendorRating(
    service?.VendorId
  );
  const { data: vendor, isLoading: vendorLoading } = useGetVendorById(
    service?.VendorId
  );
  const { data: reviews, isLoading: reviewsLoading } = useGetVendorReviews(
    service?.VendorId
  );
  ...
  const images = vendorService?.[0]?.media ?? [];
  ...
  useEffect(() => {
    if (followers && user) {
      const follow = followers.some(
        (follow: any) => follow.followerId === user.id
      );
            ...
            <div className="flex flex-col items-center sm:items-start gap-2">
              <h6 className="flex items-center gap-2">
                ...
                {service?.Vendor?.status === "Approved" && (
                  <BadgeCheck fill="#b30909" className="text-white" />
                )}
              </h6>
              <div className="flex items-center gap-[6px]">
                ...
                >
                  ...
                  <p className="text-[14px] text-white font-[400] pl-2">
                    {rating?.vendorRating?.completionPercentage || "--"}{" "}
                    Completion
                  </p>
                </span>
                <span className="flex items-center">
                  <Star
                    fill="#F48E2F"
                    className="h-5 w-5 text-[#F48E2F] ml-2"
                  />
                  <p className="text-[#F48E2F] font-[500] pl-1">
                    {rating?.vendorRating?.rating || 0}
                  </p>
                </span>
              </div>
              <span className="flex items-center">
                    ...
                    {activeService?.map((store: any) => (
                      <TabsContent value={store.name} key={store.name}>
                        <div className="flex flex-col gap-5 mt-[10px] max-w-[1000px]">
                          <div>
                            <p className="text-black font-[500] mb-1">
                              Package name
                            </p>
                                  ...
                                  (image: string, index: number) => (
                                    ...
                                    >
                                      <Image
                                        ...
                                        height={400}
                                        alt="Image"
                                        className="w-[200px] sm:w-[350px] object-cover h-[200px] sm:h-[300px] rounded-lg flex-shrink-0 hover:opacity-90 transition"
                                      />
                                    </button>
                                  )
                                )}
                          ...
                          <Button
                            className="mt-5 mx-auto w-fit gap-2"
                            onClick={() =>
                              navigation.push(`${marketId}/${store.name}`)
                            }
                          >
                            <Package size={20} />
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 py-10 border-b border-gray-200">
                            {displayedComments?.map(
                              (review: any, index: number) => (
                                <div
                                  key={index}
                                  className="flex flex-col gap-3 border-b border-gray-200 pb-5"
                                >
                                  <div className="flex gap-3">
                                    <Image
                                      src={
                                        review?.User?.avatar || "/noavatar.png"
                                      }
                                      width={100}
                                      height={100}
                                      alt="Avatar"
                                    ...
                                    <div>
                                      ...
                                      <span className="flex gap-[2px]">
                                        {Array.from({
                                          ...
                                          <Star
                                            fill="#F48E2F"
                                            className="text-[#F48E2F] h-4 w-4 m-0"
                                            key={index}
                                          />
                                        ))}
                                      </span>
                                  ...
                                  <div>
                                    <p className="text-black font-[500] mb-2">
                                      Description
                                    </p>
                                    <p>{review?.comment}</p>
                                  </div>
