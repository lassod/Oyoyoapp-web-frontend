  ...
  useEffect(() => {
    if (bookmarks.length > 0) {
      const isBookmarkedByUser = bookmarks.some(
        (bookmark: any) => bookmark.eventId === item.id
      );
      ...
      <div className="relative grid grid-cols-4 justify-between px-3 py-5 bg-gray-50">
        <div className="flex gap-[20px] col-span-3">
          ...
          <div className="w-[190px] flex flex-col gap-2">
            <h5 className="text-[13px] font-[500] line-clamp-1">
            ...
            <span className="flex gap-2 text-[12px] font-[400]">
              ...
              {item?.address === "undefined" || item?.address === "Online"
                ? "Virtual"
                : shortenText(item?.address, 20)}
            </span>
            <div className="avatar flex items-center gap-2">
              <span className="flex">
                {item?.Event_Attendees?.slice(0, 3).map(
              ...
              <p className="text-[11px]">
                +{item?.Event_Attendees?.length || 0} is going
              </p>
            </div>
          </div>
        </div>
  ...
  const handleEventClick = (data: any, value: string) => {
    ...
    if (guest) {
      ...
      if (value === "upcoming" || value === "past") {
        ...
        const isAttending = attending?.find(
          (attendingEvent: any) =>
            String(attendingEvent?.id) === String(data?.id)
        );
        if (isAttending)
          router.push(`/dashboard/events/completed?id=${data?.id}`);
...
export const highlightMatch = (text = "", q = "", extra = "") => {
  if (!q) return <span className={extra}>{text}</span>;
  const lower = text.toLowerCase();
  const qi = q.toLowerCase();
  const idx = lower.indexOf(qi);
  if (idx < 0) return <span className={extra}>{text}</span>;
  return (
    <span className={extra}>
      {text.slice(0, idx)}
      <mark className="bg-red-600">{text.slice(idx, idx + q.length)}</mark>
