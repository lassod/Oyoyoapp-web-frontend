...
import { Settings, Search, Bell, MessageSquareMore } from "lucide-react";
...
import { useGetNotificationsUnread } from "@/hooks/notification";

const Navbar = ({ setIsSearch, isSearch }: any) => {
  const { data: user } = useGetUser();
  const { data: session } = useSession();
  const [isNotification, setIsNotification] = useState(false);
  const { data: notificationsData } = useGetNotificationsUnread();
