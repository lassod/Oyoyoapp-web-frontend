...
const SignupEmail = ({ personal }: { personal: boolean }) => {
  ...
  const onSubmit = (values: z.infer<typeof formSignUp>) => {
    ...
    if (personal) {
      ...
      const category = allCategories?.data?.find((item: any) => values.categoryId === item.name);
      const newBusiness = {
        ...values,
  ...
  return (
    <div className='bg-white w-full max-w-[600px] rounded-[20px] px-4 sm:px-[30px] py-[50px]'>
      ...
      <p className='text-black font-medium'>
        ...
        <Link className='text-red-700 font-medium hover:text-black' href='/auth/login'>
          Sign In
        </Link>
      </p>
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className='flex flex-col mt-4 gap-2'>
          <FormField
            name='email'
            render={({ field }) => (
              <FormItem>
                    ...
                    <Popover open={isOpenCategory} onOpenChange={setIsOpenCategory}>
                      <PopoverTrigger asChild>
                        <Button
                          ...
                          role='combobox'
                          size={"sm"}
                          className={cn(!field.value && "text-gray-400 font-normal")}
                        >
                          {field.value
                            ? allCategories?.data?.find((category: { name: string }) => category.name === field.value)
                                ?.name
                      ...
                      <PopoverContent className='w-full p-0'>
                        <Command>
                          ...
                          <CommandList>
                            ...
                            <CommandGroup>
                              {allCategories?.data?.map((category: { name: string }) => (
                                <CommandItem
                                  value={category.name}
                                  key={category.name}
                                  onSelect={() => {
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <div className='relative flex justify-between w-full'>
                  <Input type={`${hide ? "password" : "text"}`} placeholder='Enter password (Ex. 1234)' {...field} />
                  {hide ? (
                    <EyeOff
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem className='mt-2'>
                ...
                <div className='relative flex justify-between w-full'>
                  <Input type={`${hide2 ? "password" : "text"}`} placeholder='Confirm password' {...field} />
                  {hide2 ? (
                    <EyeOff
                      className='absolute right-3 text-gray-600 top-2 bg-white pl-1'
