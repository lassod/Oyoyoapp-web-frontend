...
import { AlertDialog, AlertDialogAction, SuccessModal, ErrorModal } from "@/components/ui/alert-dialog";
  ...
  return (
    <section className='min-h-screen flex items-center justify-center py-[50px]'>
      ...
      <div className='bg-white w-full max-w-[500px] rounded-[20px] px-4 sm:px-[30px] py-[50px]'>
        ...
        <p className='mb-2'>Your new password must be different to previously used passwords.</p>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className='flex flex-col mt-3 gap-4'>
            <FormField
              name='token'
              render={({ field }) => (
                <FormItem>
                  <FormLabel className='relative top-1'>Verification code</FormLabel>
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <div className='relative flex justify-between w-full'>
                    <Input type={`${hide ? "password" : "text"}`} placeholder='Choose password (Ex. 1234)' {...field} />
                    {hide ? (
                      <EyeOff
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel className='relative top-1'>Comfirm password</FormLabel>
                  <div className='relative flex justify-between w-full'>
                    <Input type={`${hide2 ? "password" : "text"}`} placeholder='Repeat password' {...field} />
                    {hide2 ? (
                      <EyeOff
                        className='absolute right-3 text-gray-600 top-2 bg-white pl-1'
            ...
            />
            <Button className='w-full mt-5' type='submit' disabled={mutation.isPending}>
      ...
      {successModal && (
        <AlertDialog open onOpenChange={(open) => setSuccessModal(open)}>
          <SuccessModal title='Password Updated Successfully!' description='Your password as successfully been updated'>
            <AlertDialogAction className='mt-4' onClick={() => navigation.push("login")}>
              Close
            </AlertDialogAction>
          </SuccessModal>
        </AlertDialog>
