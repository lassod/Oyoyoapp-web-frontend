...
export default function StripeReturn() {
        ...
        const interval = setInterval(() => {
          setProgress((prevProgress) => {
            ...
            if (prevProgress % 20 === 0) {
              setWaitMessageIndex((prevIndex) => (prevIndex + 1) % waitMessages.length);
            }
  ...
  const LoadingScreen = () => (
    <Card className='w-full max-w-md overflow-hidden border-0 shadow-lg'>
      <CardHeader className='bg-white pb-0'>
        <CardTitle className='text-2xl font-bold text-gray-800'>Stripe Onboarding</CardTitle>
      </CardHeader>
      <CardContent className='flex flex-col items-center justify-center space-y-6 p-6'>
        <Loader2 className='w-16 h-16 text-red-600 animate-spin' />
        <p className='text-xl text-center text-gray-700'>
          {status === "loading" ? "Processing your onboarding..." : "Processing failed."}
        </p>
        {status === "loading" && (
  ...
  const SuccessScreen = () => (
    <Card className='w-full max-w-md overflow-hidden border-0 shadow-lg'>
      <CardHeader className='bg-white pb-0'>
        <CardTitle className='text-2xl font-bold text-gray-800'>Stripe Onboarding Complete</CardTitle>
      </CardHeader>
      <CardContent className='flex flex-col items-center justify-center space-y-6 p-6'>
        <motion.div
          initial={{ scale: 0 }}
        ...
        <p className='text-xl text-center text-gray-700'>
          Congratulations! Your Stripe account has been successfully set up and integrated.
        </p>
        {showAlert && (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
            <Alert className='bg-green-50 border border-green-200 text-green-800'>
              <AlertTitle>Success</AlertTitle>
              <AlertDescription>You can now close this window. The onboarding process is complete.</AlertDescription>
            </Alert>
          </motion.div>
        )}
      </CardContent>
  ...
  const ErrorScreen = () => (
    <Card className='w-full max-w-md overflow-hidden border-0 shadow-lg'>
      <CardHeader className='bg-white pb-0'>
        <CardTitle className='text-2xl font-bold text-gray-800'>Stripe Onboarding Failed</CardTitle>
      </CardHeader>
      <CardContent className='flex flex-col items-center justify-center space-y-6 p-6'>
        ...
        <Alert className='bg-red-50 border border-red-200 text-red-800'>
          ...
          <AlertDescription>{errorMessage || "Please try again or contact support for assistance."}</AlertDescription>
        </Alert>
      </CardContent>
      <CardFooter className='flex flex-col gap-4 p-6 bg-gray-100'>
        <Button
